...
if ($env:APPVEYOR_REPO_BRANCH -ine 'main' -or $env:APPVEYOR_REPO_BRANCH -ine 'dev') {
    ...
    if ($res.FailedCount -eq 0) {
        ...
        foreach ($moduleItem in $moduleData) {
            ...
            try {
                ...
                if ($env:APPVEYOR_REPO_BRANCH -ieq 'dev') {
                    $Params['NuGetApiKey'] = $env:NugetApiTestKey
                    $textToShow = "PowerShell Gallery (test mode)."
                }
                if ($env:APPVEYOR_REPO_BRANCH -ieq 'main') {
